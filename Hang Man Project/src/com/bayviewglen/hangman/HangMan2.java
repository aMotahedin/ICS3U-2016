	...
	public static void main(String[] args) {
		...
		while (!gameOver) {
			...
			while (!roundOver) { // two rounds are needed to complete an
				...
				while (!validPhrase) {
					System.out.println("\n" + "\n" + currentPhraseMaker + " Please enter a phrase for "
							+ currentPhraseGuesser + " to solve");
				...
				if (!roundOver && numberOfGuesses == MAX_GUESSES) {
					...
					if (numberOfGuesses == MAX_GUESSES && forcedGuess != phrase) {
						System.out.println(currentPhraseGuesser + ", you have lost this round");
						round++;
						roundOver = true;
					} else
				...
				if (player1Name == currentPhraseGuesser) {
					...
					playerTwoScore += MAX_GUESSES - numberOfGuesses;
				}
				System.out.println(player1Name + "'s score is " + playerOneScore + " and " + player2Name
						+ "'s score is " + playerTwoScore);
